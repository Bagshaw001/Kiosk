	...
	if ($request == "POST") {
		...
		switch ($_POST["action"]) {
				...
				if (is_user_a_collaborator($store_id, $manager_id)) {
					...
					if ($is_wallet) {
						...
						if ($enough_balance) {
							...
							send_json(
								array("msg" => "Store does not have enough balance"),
								100
							);
						}
					} else {
						...
						send_json(
							array("msg" => "Store does not have a wallet with matching details"),
							100
						);
					}
				} else {
					send_json(
						array("msg" => "User is not a collaborator for the store"),
						100
					);
				}
			...
			case "schedule_post":
				...
				$time_to_post = $_POST["time_to_post"];
				$post_id = $_POST["post_id"];


				$success = add_schedule_post($post_id, $caption, $time_to_post);

				if ($success) {
					send_json(array("msg" => "Successfully added post"));
				} else {
					send_json(array("msg" => "You may already have this post scheduled"), 100);
				}

				die();
			case "delete_schedule_post":
				$post_id = $_POST["post_id"];

				echo (get_schedule_post($post_id));

				if (get_schedule_post($post_id) == false) {
					send_json(array("msg" => "Post does not exist"), 100);
				} else {
					$success = remove_schedule_post($post_id);
					if ($success) {
						send_json(array("msg" => "Successfully deleted post"));
					} else {
						send_json(array("msg" => "Error occured whilst deleting post"), 100);
					}
				}
			...
			case "get_schedule_post":
				...
				$post_info = get_schedule_post($post_id);

				send_json(array("wallets" => $post_info));
